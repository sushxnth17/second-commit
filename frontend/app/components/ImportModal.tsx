...
export default function ImportModal({ onClose, onImportSuccess }: ImportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="flex h-full max-h-[80vh] w-full max-w-xl flex-col rounded-md border border-zinc-900 bg-zinc-950 shadow-2xl transition-colors overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-zinc-900 p-5">
          ...
          <button
            onClick={onClose}
            className="rounded p-1.5 text-zinc-550 hover:bg-zinc-900 hover:text-white transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-indigo-500"
          >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <div className="relative mb-4">
            <input
              ...
              className="w-full rounded border border-zinc-900 bg-zinc-955 py-2 pl-9 pr-4 text-xs text-white outline-none placeholder:text-zinc-550 transition-colors focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus-visible:ring-1 focus-visible:ring-indigo-500 focus-visible:border-indigo-500"
            />
            <svg
          ...
          {loading ? (
            ...
            <div className="flex h-48 flex-col items-center justify-center text-center">
              ...
              <span className="mt-1 text-[10px] text-zinc-650 font-mono">Try adjusting your search query.</span>
            </div>
          ) : (
            <div className="grid gap-2">
              {filteredRepos.map((repo) => {
                ...
                return (
                  <div
                    key={repo.id}
                    className="flex items-center justify-between gap-4 rounded border border-zinc-900 bg-zinc-950/20 p-2.5 transition-colors hover:bg-zinc-900/10"
                  >
                    ...
                    <button
                      ...
                      className={`rounded px-2.5 py-1.5 text-[10px] font-semibold transition-all shrink-0 cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-indigo-500 ${
                        isImported
        ...
        <div className="border-t border-zinc-900 bg-zinc-950 p-4 flex justify-end">
          <button
            onClick={onClose}
            className="rounded border border-zinc-800 bg-zinc-900/30 px-3.5 py-1.5 text-[10px] font-semibold text-zinc-400 hover:text-white hover:bg-zinc-900 hover:border-zinc-700 transition-all cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-indigo-500"
          >
            Close
          </button>
        </div>
