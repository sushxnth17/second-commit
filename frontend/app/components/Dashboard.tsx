...
export default function Dashboard({
  ...
  const getHealthBadgeColor = (grade?: string) => {
    switch (grade) {
      ...
      case "B":
        return "text-teal-400 border-teal-950 bg-teal-950/20";
      case "C":
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12">
      ...
      <div className="mb-12 flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-serif text-white tracking-tight font-light">
      ...
      <div className="grid gap-12 md:grid-cols-5 border-t border-b border-zinc-900 py-8 mb-12">
        ...
        <div className="md:col-span-3 flex flex-col justify-between pr-0 md:pr-10 border-r-0 md:border-r border-zinc-900/60">
          <div>
            <h3 className="text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-widest mb-5">Repository Health</h3>

            <div className="flex items-center gap-4 mb-6">
              <span className="text-5xl font-mono text-white tracking-tighter leading-none font-normal select-none">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-zinc-500 font-mono leading-none select-none">Average Health</span>
                <span className="text-[9px] font-mono text-zinc-400 mt-1 select-none">Grade {
                  analytics?.average_health_score !== null && analytics?.average_health_score !== undefined
          ...
          {/* Distribution list */}
          <div className="space-y-2.5 text-xs text-zinc-500 max-w-sm">
            {(["A", "B", "C", "D", "F"] as const).map((grade) => (
              <div key={grade} className="flex items-center gap-3">
                <span className="w-3 font-mono text-[10px] text-zinc-500 text-right select-none">{grade}</span>
                <div className="h-[1px] flex-1 bg-zinc-900/80">
                  <div
                ...
                <span className="w-4 text-right font-mono text-[9px] text-zinc-500 select-none">
                  {gradeCounts[grade]}
                </span>
              </div>
            ))}
            <div className="mt-6 pt-4 text-[10px] font-mono uppercase tracking-wider text-zinc-550 border-t border-zinc-900/60">
          ...
          <div>
            ...
            <div className="divide-y divide-zinc-900/60">
              <div className="flex justify-between py-3">
                ...
                <span className="text-xs font-mono text-zinc-300 tracking-tight">{repos.length}</span>
              </div>
              <div className="flex justify-between py-3">
                ...
                <span className="text-xs font-mono text-zinc-300 tracking-tight">
                  ...
                  <span className="text-zinc-705 mx-1.5">/</span>
                  <span className="text-rose-450">{analytics?.dormant_repositories ?? 0}</span>
                </span>
              </div>
              <div className="flex justify-between py-3">
                <span className="text-xs text-zinc-400">Total Stars</span>
                <span className="text-xs font-mono text-zinc-300 tracking-tight">{analytics?.total_stars ?? 0}</span>
              </div>
              <div className="flex justify-between py-3">
                <span className="text-xs text-zinc-400">Total Forks</span>
                <span className="text-xs font-mono text-zinc-300 tracking-tight">{analytics?.total_forks ?? 0}</span>
              </div>
            </div>
          </div>
      ...
      {reposNeedingAttention.length > 0 && (
        <div className="mb-12">
          ...
          <div className="border border-zinc-900 bg-zinc-950/20 divide-y divide-zinc-900/60">
              ...
              >
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  <div className="mt-1.5 flex items-center gap-3">
                    {repo.language && (
                      <span className="rounded-none bg-zinc-900 border border-zinc-850 px-2 py-0.5 text-[9px] text-zinc-400 font-mono">
                        {repo.language}
                      </span>
                    )}
                ...
                <div className="flex items-center gap-5 shrink-0">
                  ...
                  <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-550 group-hover:text-zinc-300 transition-colors pl-2 select-none">
                    View →
                  </span>
                </div>
              </div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between">
        <div>
          ...
          <p className="text-[10px] text-zinc-550 mt-1">Explore metrics, stars, and language details for your codebase.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative">
            <input
              ...
              className="w-full sm:w-48 rounded-none border border-zinc-900 hover:border-zinc-800 bg-zinc-950 py-1.5 pl-8 pr-3 text-xs text-zinc-400 outline-none transition-colors focus:border-zinc-700"
            />
            <svg
          ...
          <select
            ...
            className="rounded-none border border-zinc-900 hover:border-zinc-800 bg-zinc-950 py-1.5 px-3 text-xs text-zinc-400 outline-none cursor-pointer focus:border-zinc-700 transition-colors"
          >
      ...
      {filteredRepos.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-none border border-dashed border-zinc-900 py-16 text-center">
          <div className="flex h-10 w-10 items-center justify-center bg-zinc-950 text-zinc-550 border border-zinc-900 mb-4 select-none">
        ...
        <div className="border border-zinc-900 bg-zinc-950/20 overflow-hidden overflow-x-auto">
          <table className="w-full min-w-[750px] border-collapse text-left text-xs text-zinc-400">
            <thead className="bg-zinc-950/60 text-[9px] uppercase font-mono tracking-widest text-zinc-500 border-b border-zinc-900 select-none">
              <tr>
                ...
                <th className="py-3.5 px-4 text-center font-semibold">Stars</th>
                <th className="py-3.5 px-4 text-center font-semibold">Forks</th>
            ...
            <tbody className="divide-y divide-zinc-900/60">
              {filteredRepos.map((repo) => (
                ...
                >
                  <td className="py-3.5 px-4 font-serif text-sm font-normal text-zinc-200 group-hover:text-white transition-colors max-w-[220px] truncate">
                    {repo.name}
                  </td>
                  <td className="py-3.5 px-4 font-mono text-[10px]">
                    {repo.language ? (
                      <span className="rounded-none bg-zinc-900 border border-zinc-850 px-2 py-0.5 text-[9px] text-zinc-400">
                        {repo.language}
                      </span>
                    ) : (
