...
export default function RepoDetails({ repoId, onBack, onSyncSuccess }: RepoDetailsProps) {
  ...
  const getScoreColor = (score: number) => {
    if (score >= 90) return "text-emerald-405 border-emerald-950/30 bg-emerald-950/10";
    if (score >= 80) return "text-teal-405 border-teal-950/30 bg-teal-950/10";
    if (score >= 70) return "text-amber-405 border-amber-950/30 bg-amber-950/10";
    return "text-rose-455 border-rose-950/30 bg-rose-950/10";
  };

  const getDormancyColor = (status: string) => {
    if (status.toLowerCase() === "active") return "text-emerald-405 border-emerald-950/30 bg-emerald-950/10";
    return "text-rose-455 border-rose-950/30 bg-rose-950/10";
  };

  if (loading) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-4 bg-zinc-955">
    ...
    return (
      <div className="mx-auto max-w-xl px-6 py-16 text-center">
        <div className="border border-red-950 bg-red-950/10 p-6 rounded-none">
          ...
          <p className="mt-2 text-xs text-red-450/85 leading-relaxed">{error || "Repository details not found."}</p>
          <button
            onClick={onBack}
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between mb-8">
        <div className="flex items-baseline gap-4">
          <button
            onClick={onBack}
            className="rounded-none border border-zinc-900 p-1.5 text-zinc-500 hover:bg-zinc-900 hover:text-white transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-zinc-650"
          >
          ...
          <div>
            <h1 className="text-2xl font-serif text-white tracking-tight">{repo.name}</h1>
        ...
        <button
          onClick={handleSync}
          disabled={syncing}
          className={`flex items-center justify-center gap-2 rounded-none border border-zinc-800 bg-zinc-900/30 px-4 py-2 text-[10px] font-mono uppercase tracking-wider text-zinc-350 hover:text-white hover:bg-zinc-900 hover:border-zinc-700 transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-zinc-650 ${
            syncing ? "cursor-not-allowed text-zinc-600 border-zinc-900/40 bg-zinc-950" : ""
          }`}
        >
          {!syncing && (
            <svg
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-5 border-t border-b border-zinc-900 mb-8">
        <div>
      ...
      <div className="grid gap-8 md:grid-cols-3 mb-8">
        {/* Health score gauge */}
        {health && (
          <div className="border border-zinc-900 bg-zinc-950/20 p-5 flex flex-col justify-between rounded-none min-h-[190px]">
            <div>
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 mt-1 font-mono">Grade: <span className="font-semibold text-zinc-355">{health.grade}</span></p>
                </div>
            ...
            <div>
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 mt-1 font-sans">Since last repository push</p>
                </div>
            ...
            <div>
              ...
              <div className="mt-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500 mt-1 font-sans">Complexity: <span className="font-semibold text-zinc-350 font-mono">{aiInsights.complexity}</span></p>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className={`rounded-none px-2 py-0.5 text-[9px] font-mono uppercase tracking-wider font-semibold border ${
                    aiInsights.beginner_friendly
                      ? "text-emerald-450 border-emerald-955/30 bg-emerald-955/10"
                      : "text-amber-455 border-amber-955/30 bg-amber-955/10"
                  }`}>
        ...
        {!aiInsights && !aiLoading && !aiError && (
          <div className="border border-zinc-900 bg-zinc-950/20 p-5 flex flex-col justify-between rounded-none min-h-[190px]">
            ...
            <button
              ...
              className="mt-5 w-full rounded-none border border-zinc-800 bg-zinc-900 px-4 py-2 text-[10px] font-mono uppercase tracking-wider text-zinc-300 hover:text-white hover:bg-zinc-800 transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-zinc-650"
            >
        ...
        {aiLoading && (
          <div className="border border-zinc-900 bg-zinc-950/20 p-5 flex flex-col justify-center items-center rounded-none min-h-[190px] text-center">
        ...
        {aiError && (
          <div className="border border-zinc-900 bg-zinc-950/20 p-5 flex flex-col justify-between rounded-none min-h-[190px]">
            <div>
              ...
              <p className="mt-3 text-xs text-rose-455/80 leading-relaxed line-clamp-3">
                {aiError}
              </p>
            </div>
            <button
              onClick={handleGenerateAI}
              className="mt-5 w-full rounded-none border border-red-955 bg-red-955/10 px-4 py-2 text-[10px] font-mono uppercase tracking-wider text-red-400 hover:text-red-350 transition-colors cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-red-655"
            >
              Retry AI Analysis
            </button>
          </div>
        ...
        <div className="border border-zinc-900 bg-zinc-950/40 p-8 rounded-none">
          ...
          <div className="grid gap-10 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-zinc-900/60">
            ...
            <div className="pr-0 md:pr-6">
              <h3 className="flex items-center gap-2 text-[10px] font-mono font-bold text-emerald-450 uppercase tracking-widest mb-4">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-555" />
                Core Strengths
              </h3>
              <ul className="space-y-3">
                {aiInsights.strengths.map((strength, idx) => (
                  <li key={idx} className="flex gap-2.5 text-xs text-zinc-405 leading-relaxed font-sans">
            ...
            <div className="pt-6 md:pt-0 pl-0 md:pl-8 pr-0 md:pr-6">
              <h3 className="flex items-center gap-2 text-[10px] font-mono font-bold text-rose-455 uppercase tracking-widest mb-4">
                <span className="h-1.5 w-1.5 rounded-full bg-rose-555" />
                Weaknesses
              </h3>
              <ul className="space-y-3">
            ...
            <div className="pt-6 md:pt-0 pl-0 md:pl-8">
              <h3 className="flex items-center gap-2 text-[10px] font-mono font-bold text-zinc-300 uppercase tracking-widest mb-4">
              ...
              <ul className="space-y-3">
                {aiInsights.suggestions.map((suggestion, idx) => (
                  <li key={idx} className="flex gap-2.5 text-xs text-zinc-405 leading-relaxed font-sans">
                    <span className="text-zinc-650 shrink-0 select-none font-mono">-</span>
